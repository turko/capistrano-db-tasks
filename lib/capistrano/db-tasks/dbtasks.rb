...
set :db_config, 'app/config/capistrano.yml' unless fetch(:db_config)
set :local_env, ENV['LOCAL_ENV'] || 'capistrano' unless fetch(:local_env)
set :db_local_clean, false unless fetch(:db_local_clean)
set :assets_dir, [] unless fetch(:assets_dir)
...
namespace :db do
  ...
  task :pull do
    on roles(:db) do
      puts "Override your Local database: #{Database::Local.new(self).database}"
      puts "With the Remote database: #{Database::Remote.new(self).database}"
...
namespace :assets do
  ...
  task :pull => 'capistrano_db_tasks:check_can_push' do
    on roles(:app) do
